...
class ViewController: UIViewController {
    ...
    @objc func button(_ sender: UIButton) {
        let alertController = UIAlertController(title: "카운트", message: "추가 or 빼기 or 리셋", preferredStyle: .alert)
        ...
        let confirm = UIAlertAction(title: "text창 값 입력", style: .default) { (_) in
    ...
    func alert1() {
        let textAlertController = UIAlertController(title: title, message: "숫자만 입력하세요", preferredStyle: .alert)
